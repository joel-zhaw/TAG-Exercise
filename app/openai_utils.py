...
# --- 4. Full Pipeline ---
def text_to_sql_qa(question: str, schema: str, engine: Engine, model = "gpt-4o-mini") -> str:
    sql = generate_sql_openai(question, schema, model=model)
    df = run_sql(engine, sql)
    answer = llm_answer_openai(question, sql, df, model=model)
    return answer
